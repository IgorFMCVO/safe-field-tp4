`timescale 1ns/1ps
module sf_telemetry_tx #(parameter integer UART_CLKS_PER_BIT=234)(input wire clk,input wire reset_n,input wire event_valid,output wire event_ready,input wire [7:0] event_type,input wire [15:0] event_sequence,input wire [31:0] event_data,input wire [7:0] event_flags,output wire uart_tx,output reg busy);
reg [7:0] frame[0:11];reg [3:0] byte_index;reg tx_start;reg [7:0] tx_data;wire tx_busy;wire tx_done;
...
always @(posedge clk or negedge reset_n) begin
 if(!reset_n)begin busy<=0;byte_index<=0;tx_start<=0;tx_data<=0;end else begin tx_start<=0;
 if(!busy&&event_valid)begin frame[0]<=8'h5A;frame[1]<=8'hA5;frame[2]<=8'h05;frame[3]<=event_type;frame[4]<=event_sequence[15:8];frame[5]<=event_sequence[7:0];frame[6]<=event_data[31:24];frame[7]<=event_data[23:16];frame[8]<=event_data[15:8];frame[9]<=event_data[7:0];frame[10]<=event_flags;frame[11]<=crc_frame(8'h05,event_type,event_sequence[15:8],event_sequence[7:0],event_data[31:24],event_data[23:16],event_data[15:8],event_data[7:0],event_flags);busy<=1'b1;byte_index<=0;end
 else if(busy)begin if(tx_done)begin if(byte_index==4'd11)begin busy<=1'b0;byte_index<=0;end else byte_index<=byte_index+1'b1;end else if(!tx_busy&&!tx_start)begin tx_data<=frame[byte_index];tx_start<=1'b1;end end
 end end
endmodule
